...
alldata = ValueReplace(alldata, vars = c("wood_consume_prop", "meat_consume_prop", "magic_consume_prop", "stone_consume_prop", "ivory_consume_prop", "resource_level1_add_value", "resource_level1_reduce_value",
                                         ...
                                         "pvp_lanch_prop", "pvp_win_prop", "pve_lanch_prop", "pve_win_prop"), ori = NaN, new = '-2')
...
alldata = ValueReplace(alldata, vars = c("wood_consume_prop", "meat_consume_prop", "magic_consume_prop", "stone_consume_prop", "ivory_consume_prop", "resource_level1_add_value", "resource_level1_reduce_value",
                                         ...
                                         "soldier_consume_prop", "wound_infantry_consume_prop", "wound_cavalry_consume_prop", "wound_shaman_consume_prop", "wound_soldier_add_value",
                                         "wound_soldier_reduce_value", "wound_soldier_consume_prop", "acceleration_add_value", "acceleration_reduce_value", "acceleration_consume_prop",
                                         "pvp_lanch_prop", "pvp_win_prop", "pve_lanch_prop", "pve_win_prop"), ori = 'Inf', new = '-1')

VarStat(alldata, VarType(alldata), nstat = c("Miss", "Mean", "Min", "Max", "Median", "Num"))
```

## (3) 分群变量
```{r}
ddply(alldata, .(), summarise, 
...
ddply(alldata, .(is_pay_price), summarise, 
      cnt = length(user_id), add_target_cnt = sum(add_target), add_target_rto = mean(add_target), target_cnt = sum(target), target_rto = mean(target), rmse = sqrt(mean((prediction_pay_price - pay_price)^2)))
```
...
## (4) 数据分群
```{r}
prepaydata = subset(alldata, is_pay_price == 1)
nonprepaydata = subset(alldata, is_pay_price == 0)
```
...
## (1) 数据描述统计
```{r}
prepaydata_iv = CatVarImp(prepaydata[, c(3:150, 154)], yvar = 'add_target', method = 'iv', bins = 10)
for (x in colnames(prepaydata)[c(3:150)]) {
  CShipPlot(prepaydata, xvar = x, yvar = 'add_target', bins = 10, pdf = T, savedir = 'D:/play/tap4fun/tap4fun/xxw_testdata/plot/prepay/') 
}

prepaydata.numericvars.index = which(VarType(prepaydata)$numeric)
prepaydata_iv; write.csv(prepaydata_iv[[2]], 'D:/play/tap4fun/tap4fun/xxw_testdata/plot/prepay/prepaydata_iv.csv', row.names = F)
prepaydata_cor = data.frame(round(cor(prepaydata[, prepaydata.numericvars.index]),2)); write.csv(prepaydata_cor, 'D:/play/tap4fun/tap4fun/xxw_testdata/plot/prepay/prepaydata_cor.csv', row.names = T)
```

## (2) 模型搭建 
```{r}
prepaydata_train.index = sample(1:nrow(prepaydata), round(nrow(prepaydata)*0.8))
prepaydata_train = prepaydata[prepaydata_train.index, ]
prepaydata_test = prepaydata[-prepaydata_train.index, ]

gbm.model = gbm(add_target ~ ., distribution = "bernoulli", data = prepaydata_train[, prepaydata.numericvars.index[c(1:107, 109:144, 148)]], 
                n.trees = 100, n.minobsinnode = 10, shrinkage = 0.001, bag.fraction = 0.5, train.fraction = 1, n.cores = 2)
summary(gbm.model)


ks.test(gbm.model$fit[prepaydata_train$add_target == 1], gbm.model$fit[prepaydata_train$add_target == 0])

CatModelPerf(datalist = list(prepaydata_train, prepaydata_test), model = gbm.model, yvar = 'add_target')
```
